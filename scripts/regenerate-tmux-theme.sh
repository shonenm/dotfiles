...
case "$THEME" in
  ...
  syntopic)
    ...
    ;;
  *)
    echo "Unknown theme: $THEME (available: tokyonight, syntopic)" >&2
    exit 1
    ;;
esac
...
SR_SYSSTAT="#[fg=${BG_HIGHLIGHT}${EC}bg=default]${LEFT}#(~/dotfiles/scripts/tmux-cpu.sh)#[fg=${FG_SUBTLE}${EC}bg=${BG_HIGHLIGHT}]|#(~/dotfiles/scripts/tmux-ram.sh)#(~/dotfiles/scripts/tmux-gpu.sh)#(~/dotfiles/scripts/tmux-storage.sh)#[fg=${BG_HIGHLIGHT}${EC}bg=default]${RIGHT} "
