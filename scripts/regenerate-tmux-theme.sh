...
HELP="#[fg=${COLOR_WARNING} bg=default]${LEFT}#[fg=${BG_DARK} bg=${COLOR_WARNING} bold] ${KEY_ICON} #[fg=${COLOR_WARNING} bg=default]${RIGHT} -| split  g git  G live-pr  k nvim  l scratch  f sess  F proj  v copy  r reload  ? keys  Space menu"
...
SR_GIT="#[fg=${COLOR_GIT}${EC}bg=default]${LEFT}#[fg=${BG_DARK}${EC}bg=${COLOR_GIT}] ${GIT_ICON} #{?@git_branch,#{@git_branch},-} #[fg=${COLOR_GIT}${EC}bg=default]${RIGHT}"
