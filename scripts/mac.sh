...
install_clipboard_relay() {
  # Bootstrap clipboard-relay-server via launchd. The server is a small
  # bash loop (scripts/clipboard-relay-server) that listens on
  # 127.0.0.1:2489 with `nc` and pipes each connection into pbcopy.
  # SSH RemoteForward 2489 (mac/ssh/.ssh/config) tunnels remote bytes here.
  local plist_src="$DOTFILES_DIR/templates/com.user.clipboard-relay.plist"
  local plist_dst="$HOME/Library/LaunchAgents/com.user.clipboard-relay.plist"
  if [[ ! -f "$plist_src" ]]; then
    log_warn "clipboard-relay plist not found: $plist_src"
    return
  fi
  mkdir -p "$HOME/Library/LaunchAgents"
  sed "s|__HOME__|$HOME|g" "$plist_src" > "$plist_dst"
  launchctl bootout "gui/$(id -u)/com.user.clipboard-relay" 2>/dev/null || true
  launchctl bootstrap "gui/$(id -u)" "$plist_dst"
  log_success "clipboard-relay-server launched via launchd (com.user.clipboard-relay)"
}
