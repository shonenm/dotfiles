...
- **対象**: `@czottmann/pi-automode` 1.15.0 の `extensions/auto-mode/classifier.ts`
...
- **対処**: 単一モデル分類器を薄いwrapperで包み、カンマ区切りのモデルを順に試す。認証・quota・API key・通信・不正応答だけ次へ倒し、parsed済みpolicy blockはそのまま fail-closed
