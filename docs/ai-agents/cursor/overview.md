...
**推奨: `pi-cursor-agent` プロバイダ** — Cursor を Agent ランタイム、Pi をホストにする。dotfiles の host overlay が prompt / usage / 重複 tool を薄くし、skill は `/name` でそのターンだけ展開する。steer / 割り込みの user メッセージでは Cursor live session を切って送り直す。
