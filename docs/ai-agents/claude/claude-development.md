...
├── agents/                 # ralph worker / reviewer
├── hooks/                  # lifecycle・通知・Ralph hooks
├── rules/                  # Claude Code固有ルール
└── skills/d-*/SKILL.md     # /d-* skills

scripts/
├── ai-notify.sh            # 共通通知入口
├── beacon                  # workspace登録shell command
├── claude-status*.sh       # 状態管理
└── ralph-*                 # Ralph orchestration

docs/ai-agents/claude/      # Claude Code利用文書
docs/ai-agents/ralph/       # Ralph利用文書
```
...
- [Ralph Pattern](../ralph/overview.md) - 自律開発ループ
