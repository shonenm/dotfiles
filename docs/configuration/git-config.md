...
`~/.git_template/hooks/d-pre-commit` にマージコンフリクトマーカー検出フックを配置。`init.templateDir` により `git init` した新規リポジトリに自動適用される。
...
```bash
# 手動でフックをコピー
cp ~/.git_template/hooks/d-pre-commit .git/hooks/
...
| `diff.mnemonicPrefix` | true | Use i/w/c (index/worktree/d-commit) instead of a/b |
...
```
1. /d-conflict-resolve (Claude Code skill) or 手動で merge 後に依頼
...
```
1. /d-conflict-resolve origin/develop --rebase
...
```
${XDG_RUNTIME_DIR:-$HOME/.cache}/conflict_session/<git-dir-hash>/  # worktree 単位でスコープ
...
`.gitconfig` に定義済み。リポジトリへの適用は `.git/info/attributes` に `* merge=conflict-driver` を書き込む。`/d-conflict-resolve --rebase` では trap 付きで自動管理される。
...
| コマンド | 動作 |
|----------|------|
| `/d-issue <title-or-description>` | AI が Issue 本文を生成 + ブランチ作成 |
| `/d-pr` | 変更を分析して AI が PR 本文を生成 + PR 作成 |
| `/d-pr --draft` | Draft PR として作成 |
| `/d-pr --base <branch>` | ベースブランチを指定して PR 作成 |

### 使用例

```bash
# 1. Issue 作成 + ブランチ準備
/d-issue Add logout button
...
両者は同じブランチ命名規約を使うため、`gf start` で始めた作業を `/d-pr` で PR 作成したり、`/d-issue` で始めた作業を `gf pr` で PR 作成することも可能。
