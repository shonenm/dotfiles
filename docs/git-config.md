...
| コマンド | 用途 |
|----------|------|
| `conflict-save` | コンフリクト発生時に base/ours/theirs を `/tmp/conflict_session/<sha>/<file>/` に保存 |
| `conflict-review` | 保存済み ours/theirs と解決済みファイルを nvim 3way diff で表示 |
| `conflict-driver` | git merge driver。rebase 中にコンフリクトを自動解消 |
| `conflict-resolve-file` | `claude -p` で単一ファイルのコンフリクトを解消 (conflict-driver の内部コマンド) |
| `validate-resolved` | プロジェクトタイプ検出 + 静的検証 (tsc/cargo/go vet/ruff) |
| `rebase-review` | rebase 完了後に `git range-diff` で変更コミットのコンフリクト解決をレビュー |
...
```
1. /conflict-resolve origin/main --rebase
   └─ .git/info/attributes に * merge=conflict-driver を一時登録 (trap でクリーンアップ)
   └─ git rebase <target>
      └─ conflict-driver が各コミットのコンフリクトを自動解消
         └─ git merge-file 成功 → そのまま通過
         └─ git merge-file 失敗 → conflict-resolve-file (Claude) → validate-resolved

2. rebase 完了後、rebase-review 実行
   └─ git range-diff で ! コミット (変更あり) を検出
   └─ /tmp/conflict_session/<sha>/ から 3-way ビューを生成
   └─ confidence/review_comment を winbar に表示

3. レビュー後のアクション選択
   └─ a: accept  m: amend  e: exit
```

### セッション保存構造

```
/tmp/conflict_session/
    type                              # 操作種別 (merge/rebase/cherry-pick)
    <sha>/<file_path>/base            # 共通祖先
    <sha>/<file_path>/ours            # 現在ブランチ
