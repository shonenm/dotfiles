...
| `rebase-review [base]` | rebase 完了後に `git range-diff` + git history からコンフリクト解決をレビュー |
...
2. rebase-review <target> 実行
   ...
   └─ git range-diff で ! コミットの元SHA・新SHAペアを検出
   └─ 各コミットのパッチ比較で変更があったファイルを特定
   └─ git show で 3 状態を取得: ours(new^) / resolved(new) / theirs(old)
   └─ セッションディレクトリには依存しない
