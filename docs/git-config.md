...
| `rebase-review [base]` | rebase 完了後に `git range-diff` で変更コミットのコンフリクト解決をレビュー |
...
1. /conflict-resolve origin/main --rebase
   ...
   └─ git rebase <target>
      └─ conflict-driver が各コミットのコンフリクトを自動解消
         ...
         └─ git merge-file 失敗 → conflict-resolve-file (Claude) → exit 0

2. rebase 完了後、rebase-review <target> 実行
