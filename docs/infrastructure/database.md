...
| `rainfrog` | TUI データベースクライアント（Rust製、Vimキーバインド対応） |
...
```bash
./install.sh  # pgcli, rainfrog, postgresql-client を自動インストール
mise use -g npm:sql-formatter
```
...
```bash
# 直接接続
rainfrog --url postgresql://postgres:postgres@localhost:5432/postgres
...
### pgcli / rainfrog フローティング起動

| キー | 動作 |
|------|------|
| `<leader>dp` | pgcli をフローティングターミナルで起動 |
| `<leader>de` | rainfrog をフローティングターミナルで起動 |
...
Neovim: `<leader>de` → rainfrog でテーブル構造を確認。
