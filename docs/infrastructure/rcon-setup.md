...
AI agent hookは`${DOTFILES_SHARED_DIR:-$HOME/.cache}/claude/status`へ状態を書き、ホスト側sidebarが読む。コンテナ内でagentを動かす場合はこのdirectoryをhostと共有する。
...
docker run -d \
  --name myproject-dev \
  -v ~/.cache/claude/status:/root/.cache/claude/status \
...
containerのprovider hookがhost共有status directoryへ書き込めていない（volume mount不足）。

```bash
# host側 / container側で同じJSONが見えること
