...
| `prefix A` (tmux-agent-sidebar.sh) | 全 AI agent の状態 | 常時表示の AI 状態サイドバー pane |

sesh の強みは「セッションが存在しない場合は zoxide/wildcard から新規作成する」機能。既存セッション切替のみなら `prefix s` / `prefix f` で十分。
...
```tmux
set -g detach-on-destroy off   # セッション閉鎖時もclientを残し、別sessionへ切り替える

bind C-f run-shell 'sesh list --icons | fzf-tmux ... | sesh connect ...'
```

`detach-on-destroy off` が無いとセッションを閉じた際にtmux client自体が終了し、別sessionへ切り替える操作を継続できない。
...
- `blacklist` には dotfiles 環境で使う一時セッション名 (`scratch` は `prefix j` のscratchpad) を入れる
...
- tmux 内外から、session / project候補を単純な`sesh list -i` pickerで選択する
- `prefix+C-f`のソース切替、`fd`検索、session削除が必要な場合はtmux popupを使う
- tmux 外では、`tmux attach`前に`Alt-s`から既存sessionへ接続できる
...
| 目的 | 手順 |
|------|------|
| 同一 session で並列作業 (window per worktree) | `wt new <branch>` — 既存の `wt` フロー、高速 |
| 独立 session で並列 Claude Code | `wt new <branch>` で worktree を作った後、`prefix C-f` → ワイルドカード経由で **別 session** として attach |
