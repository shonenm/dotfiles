...
| `~/.ssh/config.d/<host>.conf` | `RemoteForward 2489` で remote → mac 接続を逆 forward (信頼ホストごとに opt-in) | host のみ |
...
RemoteForward は信頼ホストごとに opt-in する (`Host *` への一括設定は、SSH 先の
侵害ホストから mac のクリップボード読取 (`lemonade paste`) や任意 URI open を
許すためセキュリティ上廃止):
