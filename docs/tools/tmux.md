...
よく使うショートカットはトップレベルにも配置（`|`, `-`, `g`, `G`, `l`, `f`, `F`, `v`, `r`, `?`）。
...
| `l` | Scratchpadシェル（永続 `scratch` セッション、トグル） | 80% × 80% |
...
- `l` のScratchpadは永続セッション (`scratch`) を使用。再度 `prefix + l` でトグル（セッション状態を維持）
...
```
  -| split  g git  G gh  k keifu  l scratch  f sess  F proj  v copy  r reload  ? keys  Space menu
```
