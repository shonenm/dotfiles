...
├── dadbod.lua             # データベース (DBUI + pgcli/rainfrog toggleterm)
...
`C-S-u`/`C-S-d` は `nowrap` 時に有効。scrollbind ウィンドウ（差分ビュー等）では全 scrollbind ウィンドウの横スクロールを同期する。
...
`:CodeDiff <rev1> <rev2>` で2つのコミット間の差分を explorer 表示する（通常経路とは独立）。この経路ではサイドバーに stage/unstage 枠を出さず、代わりにファイル単位のレビュー印を付けられる。
...
| `[`   | 前の hunk へ（sidebar focus を維持） |
| `}`   | 次の未チェックファイルへ            |
| `{`   | 前の未チェックファイルへ            |

Diff ビュー（`,`/`.` はサイドバーと同じくファイル移動。hunk 移動は `[`/`]`）:
...
コンフリクトファイルは HEAD vs ワーキングツリー（コンフリクトマーカー付き）のインライン diff として表示される。git-conflict.nvim がマーカーを検出・ハイライトし、`co`/`ct`/`cb`/`c0` で個別解決できる。diff ビューにフォーカス中、ヘルプラインにこれらのキーマップが表示される。
