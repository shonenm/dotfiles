...
pi 本体に WebFetch / WebSearch ツールは存在しない。AGENTS.md は Jina AI 経由を推奨:

```bash
curl -fsSL 'https://r.jina.ai/https://example.com'   # WebFetch (markdown)
curl -fsSL 'https://s.jina.ai/Claude%20Code'         # WebSearch
```

API key なしで 20 RPM 制限あり。

```bash
export JINA_API_KEY=jina_xxx
```

`.envrc.local` などローカル環境変数で設定 (リポジトリに含めない)。
