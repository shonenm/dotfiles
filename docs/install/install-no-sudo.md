...
プリビルドCLIは `config/mise-linux.toml` を正本として、miseのaqua / github backendで導入する。sudoの有無にかかわらずmise管理となり、`github_release` の独自インストール処理は使用しない。
...
`config/mise-linux.toml` の `aqua:neovim/neovim` から導入する。他のプリビルドCLIと同じmise shim経由で利用する。

### 1Password CLI

- sudo モード: apt (`1password-cli` パッケージ) で `/usr/bin/op`
- no-sudo モード: `cache.agilebits.com` からzipを取得し `~/.local/bin/op` に展開 (`install.sh:_install_1password_cli_user_scope`)。update APIから最新版を解決し、取得できない場合は2.33.1へフォールバックする。`OP_CLI_VERSION` で上書き可能
...
- `git`
- `curl`
- `unzip`（1Password CLIをpackage setup前に展開するため必須）
...
- [インストールガイド](./index.md) — 通常のinstall手順
- [rcon](../infrastructure/rcon.md) — リモート接続コマンド
