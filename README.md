...
既に dotfiles をインストール済みの環境で、最新版に更新する場合:

```bash
cd ~/dotfiles
git pull

# Claude Code の stow 構造をマイグレーション (必要な場合のみ)
./scripts/migrate-claude-stow.sh

# 設定を再適用
./install.sh
```

`migrate-claude-stow.sh` は、旧バージョンの dotfiles で `~/.claude` がディレクトリ全体のシンボリックリンクになっていた環境向けのマイグレーションスクリプトです。このスクリプトは:
- ランタイムファイル（認証情報、履歴、キャッシュ等）をバックアップ
- ディレクトリシンボリックを削除
- 個別ファイルシンボリックリンク構造に再構築

新規インストールの場合や、既に個別シンボリックリンク構造になっている場合は実行不要です。
