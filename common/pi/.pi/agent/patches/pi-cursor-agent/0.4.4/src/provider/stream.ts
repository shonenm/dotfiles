...
import {
  applyCursorUsage,
  rememberCursorTokenDetails,
} from "./cursor-usage";
import { shouldReuseLiveCursorSession } from "./live-session-policy";
...
async function consumeUntilBoundary(
  ...
  while (true) {
    if (signal?.aborted) {
      ...
      return { reason: "aborted", tools: [] };
    }

    const event = await Promise.race([channel.next(), aborted]);
    if (event === "aborted") {
      finalizeAllContent(contentState, output, stream);
      return { reason: "aborted", tools: [] };
    }
    ...
    try {
      ...
      if (!session) {
        ...
        const checkpointHandler = createCheckpointHandler(
          (checkpoint: ConversationStateStructure) => {
            rememberCursorTokenDetails(checkpoint);
      ...
      if (result.reason === "aborted" || options?.signal?.aborted) {
        liveSession.abort("Interrupted by user");
