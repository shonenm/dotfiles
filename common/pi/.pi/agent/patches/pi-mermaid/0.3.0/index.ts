...
export default function (pi: ExtensionAPI) {
	...
	const renderBlocks = async (
		...
		for (const [index, block] of blocks.slice(0, MAX_BLOCKS).entries()) {
			...
			const includeSource = options.includeSourceInContext ?? false;
			const contextContent = buildContextContent(block, diagramHash, issues, includeSource);
			// Cursor rejects an empty last user turn. Custom messages become user
			// messages in convertToLlm, so never send blank content or trigger a turn.
			pi.sendMessage(
				{
					customType: MESSAGE_TYPE,
					content: contextContent.trim() ? contextContent : "(mermaid)",
					display: true,
					details,
				},
				{ triggerTurn: false },
	...
	pi.on("input", async () => {
		// Do not render on user submit. That prepends a custom message and, with
		// Cursor, can start a second turn whose last user text is empty.
