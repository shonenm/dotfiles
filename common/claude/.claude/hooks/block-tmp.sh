#!/usr/bin/env bash
# block-tmp.sh - shell guard for Claude PreToolUse(Bash) and Cursor beforeShellExecution
# Bash / shell command が /tmp を literal で触れたら deny する。
...
# Claude PreToolUse は tool_name を持つ。Cursor beforeShellExecution は常に shell。
