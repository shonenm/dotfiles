...
`/_ralph-parallel` で作成された worktree、tmux window、`ralph/*` ブランチ、結果ファイルを削除するスキル。全ての作業が終わった後にユーザーが呼び出す。

## 使い方

```
/_ralph-cleanup                    # 全削除 (worktree + window + branch + results + prompts + checkpoint)
/_ralph-cleanup --keep-results     # results ディレクトリを保持
/_ralph-cleanup T-1 T-3            # 指定タスクのみ cleanup
```
