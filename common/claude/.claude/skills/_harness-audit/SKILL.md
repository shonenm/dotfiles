...
```
/_harness-audit                           # 現在のプロジェクトを全項目監査
/_harness-audit project hooks             # プロジェクトのフックのみ監査
/_harness-audit dotfiles                  # dotfiles のハーネスを全項目監査
/_harness-audit dotfiles skills           # dotfiles のスキルのみ監査
/_harness-audit scaffold                  # 新プロジェクト用ハーネスをスキャフォルド
```
...
- dotfiles scope の監査結果で「プロジェクト側に export すべき」と判断した項目は `/_claude-sync export` への誘導を行う
