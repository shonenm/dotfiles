...
when_to_use: "Use when the user asks to commit, save, or checkpoint their work. Also use proactively after completing a feature or bug fix within the session. Do NOT use when the user asks to push or create a PR — use /_pr for that."
---
...
```bash
/_commit                      # セッション内の全変更
/_commit all                  # すべての未コミット変更
/_commit src/                 # セッション内の src/ 以下のみ
/_commit all src/             # src/ 以下のすべての変更
/_commit file.ts utils.ts     # 指定ファイルのみ（セッション内）
/_commit all common/nvim/     # common/nvim/ 以下のすべての変更
```

## 重要: 対象範囲

### デフォルトモード (`/_commit`, `/_commit <path>`)
...
### 全変更モード (`/_commit all`, `/_commit all <path>`)
...
1. **引数の解析**:
   ...
   - 例: `/_commit all src/ lib/` → 全変更モード + `src/`, `lib/` のみ対象
