...
ユーザーとの対話を通じて要件定義・受入条件・設計・タスク分解を行い、`/d-ralph` で使用する状態ファイルを生成します。
...
3. Phase 3 で状態ファイルを生成したら、このスキルは終了する。実装作業は一切行わない。コードの変更、ファイルの作成・編集は禁止。実装は別セッションで `/d-ralph` を実行して行う。
