...
`/d-ralph-parallel` で完了した worker の成果物を回収するスキル。ユーザーがレビュー後に呼び出す。
...
使用例:
```
/d-ralph-collect send T-1 "変更のサマリーを出力して"
/d-ralph-collect send T-2 "この関数のエッジケースのテストを追加して"
/d-ralph-collect send T-3 "git diff を見せて"
```
